-- The demo template org's weak-topic catalog (added after the fact — see
-- scripts/seed-demo-org.ts) was never copied into a cloned demo org, so the
-- "Add a weak topic…" dropdown on both the head's and the assistant's Weak
-- Topics tab was permanently empty in every tour session: nothing to pick,
-- nothing to demonstrate. Copies topic_catalog (remapped to the clone's own
-- course ids via _course_map) and its topic_materials into the clone,
-- alongside everything else start_onboarding_demo() already clones.
