-- Security fix, caught by get_advisors right after 0059 shipped:
-- `p_profile_id != auth.uid()` evaluates to NULL (not TRUE) when auth.uid()
-- is null — i.e. an anonymous/unauthenticated caller — and `if NULL then`
-- is treated as false in plpgsql, so an anonymous caller could pass ANY
-- real profile id and silently bypass the intended authorization check
-- entirely. Fixed to explicitly reject a null auth.uid() too.
--
-- The revoke/grant below is defense in depth on top of that: Postgres
-- grants EXECUTE to PUBLIC by default for new functions, and PUBLIC is
-- inherited by both `anon` and `authenticated` — revoking only from `anon`
-- (as a first attempt) left the anon role able to call these functions
-- anyway. Revoking from PUBLIC and re-granting to `authenticated` only
-- closes that.
...
revoke execute on function public.start_onboarding_demo(uuid, text) from public;
revoke execute on function public.exit_onboarding_demo(uuid, boolean) from public;
grant execute on function public.start_onboarding_demo(uuid, text) to authenticated;
grant execute on function public.exit_onboarding_demo(uuid, boolean) to authenticated;
