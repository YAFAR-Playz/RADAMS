-- Onboarding tour: lets a real user temporarily clone a rich, pre-populated
-- "ZAD-AMS Demo" organization into a private, disposable copy scoped to
-- their own profile, walk through a guided tour against real (but throwaway)
-- data, then throw the copy away and return to their real org. Real writes,
-- real code paths, zero risk to the shared demo template or to any other
-- org — see the onboarding-tour plan for the full design.
--
-- The demo template org itself is provisioned separately by
-- scripts/seed-demo-org.ts using this fixed id: 8cfc8e75-4211-427e-b1b7-09d3b786c1ac

alter table public.profiles
  add column if not exists onboarding_tour_status text not null default 'pending'
    check (onboarding_tour_status in ('pending', 'completed', 'skipped')),
  add column if not exists pre_demo_org_id uuid null references public.organizations(id) on delete set null;

-- Clones the demo template org's full dataset into a brand-new org, links
-- the requesting profile into it according to their role (so their own
-- dashboard shows real "my courses"/"my students" rather than a colleague's),
-- and repoints their profile at the clone. profiles.org_id is read explicitly
-- by nearly every server action in this app (not just RLS), so this one
-- column swap is sufficient to make the entire app show the clone —
-- confirmed against src/lib/actions/students.ts and staff.ts before writing
-- this function.
...
declare
  ...
  v_real_org_id uuid;
begin
  -- SECURITY DEFINER runs as the function owner, not the caller, so without
  -- this check any authenticated user could pass an arbitrary p_profile_id
  -- and swap a stranger's org out from under them.
  ...
  -- assistant_id / head_id / created_by / logged_by keep pointing at the
  -- template's permanent "colleague" profiles — those are shared across
  -- every clone, never duplicated (see seed-demo-org.ts).
  ...
  -- Link the real touring user into the clone as themselves, so their own
  -- dashboard/tabs show "my courses"/"my students" rather than a colleague's.
  if p_role = 'head' then
    insert into offering_heads (offering_id, head_id)
...
-- Restores the real user's own org and deletes their disposable clone —
-- cascading foreign keys (all already ON DELETE CASCADE from
-- organizations.id down through courses/offerings/students/etc., confirmed
-- against the live schema before writing this) clean up every cloned row
-- and the temporary offering_heads/offering_assistants links added for them
-- in one statement.
create or replace function public.exit_onboarding_demo(p_profile_id uuid, p_mark_completed boolean)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  select org_id, pre_demo_org_id into v_demo_org_id, v_home_org_id
  ...
  update profiles
  set org_id = v_home_org_id,
      pre_demo_org_id = null,
      onboarding_tour_status = case when p_mark_completed then 'completed' else onboarding_tour_status end
  where id = p_profile_id;

  delete from organizations where id = v_demo_org_id;
end;
$$;
