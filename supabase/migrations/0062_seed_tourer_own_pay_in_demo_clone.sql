-- The demo clone's salary_lines are copied straight from the template org,
-- so they're only ever attributed to the template's fixed colleague payees
-- (Priya Nair, Fatima, etc.) — never to whichever real user is actually
-- touring. That leaves the tourer with zero salary_lines of their own in
-- every clone, so My Pay's "released" check (every line for this payee in
-- this period has released_at set) sees an empty set and falls back to the
-- "hasn't been released yet" empty state — the My Pay tour step then has
-- nothing to spotlight. Give the tourer their own released line, in the
-- same period as the rest of the demo data, for the head/assistant roles
-- whose tours actually visit My Pay.
...
begin
  ...
  -- Most recent period already present in the clone's own (just-copied)
  -- salary_lines — matching it means the tourer's own line lands on the
  -- same period getMyPay() already resolves to (periods[0], newest-first),
  -- rather than risking a period nothing else in the demo uses.
