...
type SignInStatus = "idle" | "loading" | "success";
...
export function LoginForm() {
  ...
  function selectMethod(m: Method) {
    startViewTransition(() => setMethod(m));
  }

  function goBack() {
    setError(null);
    startViewTransition(() => setStep((s) => (s === "sent" ? "password" : "contact")));
  }

  function handleContinue() {
    if (!contactValid) return;
    setError(null);
    startViewTransition(() => setStep("password"));
  }
  ...
  async function sendResetCode() {
    ...
    try {
      ...
      startViewTransition(() => setStep("sent"));
    } catch {
      setError("Something went wrong. Please try again.");
    } finally {
      setLoading(false);
  ...
  return (
    <div className="w-full">
      <ViewTransition key={step} enter="form-step" exit="form-step" default="none">
        {step === "contact" && (
          <>
            ...
            <div className="flex gap-1 bg-[var(--surface2)] border border-[var(--border)] rounded-[10px] p-[3px] mb-[18px]">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-[7px] h-[38px] rounded-[8px] text-[13.5px] font-semibold cursor-pointer transition-[background,color,box-shadow] duration-150"
                style={{
            ...
            <ViewTransition key={method} enter="form-step" exit="form-step" default="none">
              {method === "email" ? (
                <>
            ...
            <button
              ...
              className="mt-[22px] w-full h-[50px] rounded-[var(--rad-sm)] text-[var(--brandfg)] text-[15px] font-semibold flex items-center justify-center gap-2 transition-[background] duration-150"
              style={{
        ...
        {step === "password" && (
          <>
            ...
            <button
              ...
              disabled={password.length < 1 || signInStatus !== "idle"}
              className="mt-4 w-full h-[50px] rounded-[var(--rad-sm)] text-[var(--brandfg)] text-[15px] font-semibold flex items-center justify-center gap-[9px] transition-[background,transform] duration-200"
              style={{
                background: signInStatus === "success" ? "#16a34a" : password.length >= 1 ? "var(--brand)" : "var(--subtle)",
                cursor: password.length >= 1 && signInStatus === "idle" ? "pointer" : "not-allowed",
                transform: signInStatus === "success" ? "scale(1.015)" : "scale(1)",
              }}
            >
